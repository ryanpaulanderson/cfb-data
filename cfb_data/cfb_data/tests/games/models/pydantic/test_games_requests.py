...
class TestScoreboardRequest:
    """Test the optional ``GET /scoreboard`` filters."""

    def test_omitted_classification_preserves_upstream_default(self) -> None:
        """Send no local value when the upstream default should apply."""
        request = ScoreboardRequest()

        assert request.model_dump(exclude_none=True, by_alias=True) == {}

    def test_accepts_documented_filters(self) -> None:
        """Accept and serialize the two documented scoreboard filters."""
        request = ScoreboardRequest(classification="fcs", conference="CAA")
    ...
    def test_rejects_unknown_filter(self) -> None:
        """Reject undocumented scoreboard query parameters."""
        with pytest.raises(ValidationError, match="Extra inputs are not permitted"):
            ScoreboardRequest.model_validate({"week": 1})
