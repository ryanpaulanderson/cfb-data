...
from cfb_data.games.pandas import CFBDGamesPandasAPI
...
def test_scoreboard_returns_validated_dataframe(
    scoreboard_response: dict[str, object],
) -> None:
    """Convert current scoreboard data through the public domain client."""
    mocked = AsyncMock(return_value=[scoreboard_response])
    with patch.object(CFBDGamesPandasAPI, "_make_request", mocked):
        api = DummyGamesPandasAPI()
        dataframe = run(api.make_request("/scoreboard"))

    mocked.assert_awaited_once_with("/scoreboard", {})
    assert isinstance(dataframe, pd.DataFrame)
    assert dataframe.loc[0, "status"] == "in_progress"
    assert dataframe.loc[0, "home_team"]["name"] == "Alabama"
