...
import pytest
from cfb_data.games.api.games_api import CFBDGamesAPI
...
def test_get_scoreboard_calls_make_request():
    """Forward validated optional scoreboard filters to the current route."""
    api = DummyAPI()
    api._make_request = AsyncMock(return_value=[{"id": 1}])

    result = run(api._get_scoreboard({"classification": "fcs", "conference": "CAA"}))

    api._make_request.assert_awaited_once_with(
        "/scoreboard", {"classification": "fcs", "conference": "CAA"}
    )
    assert result == [{"id": 1}]
...
def test_route_map_contains_registered_paths():
    """Discover every route in the official Games category."""
    api = DummyAPI()
    assert set(api._route_map) == {
        "/calendar",
        "/game/box/advanced",
        "/games",
        "/games/media",
